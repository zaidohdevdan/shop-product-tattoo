...
async function CategoriesDashboardContent({ searchParamsPromise }: { searchParamsPromise: Promise<{ page?: string }> }) {
...
function CategoriesSkeleton() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-pulse">
      <div className="lg:col-span-1 h-80 bg-white rounded-[2rem] border border-slate-100" />
      <div className="lg:col-span-2 h-[500px] bg-white rounded-[2rem] border border-slate-100" />
    </div>
  );
}
  ...
  return (
    <div className="p-10 max-w-[1600px] mx-auto space-y-10 selection:bg-indigo-600 selection:text-white">
      ...
      <Suspense fallback={<CategoriesSkeleton />}>
        <CategoriesDashboardContent searchParamsPromise={searchParams} />
      </Suspense>
    </div>
  );
}
